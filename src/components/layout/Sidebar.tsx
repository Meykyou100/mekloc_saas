...
function SidebarContent({ onClose, onHelp }: { onClose?: () => void; onHelp: () => void }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto overscroll-contain rounded-[28px] border border-[var(--app-border)] bg-[linear-gradient(180deg,#fffdf7,#f8f3e9)] pb-[max(0.5rem,env(safe-area-inset-bottom))] text-[#171717] shadow-[0_18px_48px_rgba(44,35,18,.12)]">
      <div className="shrink-0 px-3.5 pb-2 pt-3.5 lg:px-3.5 lg:pt-3.5">
        <div className="flex items-center justify-between gap-3">
        <NavLink to="/dashboard" className="group flex min-w-0 items-center gap-2.5 rounded-2xl border border-[#e9e2d6] bg-white px-2.5 py-2 shadow-[0_8px_22px_rgba(44,35,18,.05)] transition hover:border-gold-300/45" onClick={onClose}>
      ...
      <nav className="grid shrink-0 gap-0.5 px-3 py-1">
        <p className="px-2 pb-0.5 pt-1 text-[10px] font-black uppercase tracking-[0.24em] text-[#927333] [@media(max-height:700px)]:hidden">
          NAVIGATION
        </p>
        {navItems
      ...
      <div className="mt-auto shrink-0 space-y-2 px-3 pb-1 pt-2">
        <div className="rounded-3xl border border-[#e6dfd3] bg-white p-3 shadow-[0_10px_28px_rgba(44,35,18,.06)]">
          ...
          <div className="mt-2 space-y-1 border-y border-[#eee8df] py-2 text-[11px]">
          ...
          <div className="mt-2 grid grid-cols-2 gap-1 text-[10px] font-bold text-[#3f3a34] [@media(max-height:760px)]:hidden">
            {['Gestion flotte', 'Contrats PDF', 'Paiements suivis', 'Rapports'].map((feature) => <span key={feature} className="rounded-lg border border-[#ece6dc] bg-[#fcfbf8] px-1.5 py-1 text-center">{feature}</span>)}
          </div>
        ...
        <button
          ...
          className="group flex cursor-pointer items-center gap-2.5 rounded-2xl border border-[#e6dfd3] bg-white p-2 shadow-[0_10px_24px_rgba(44,35,18,.05)] transition hover:border-gold-300/45 hover:bg-[#fffaf0] [@media(max-height:640px)]:hidden"
        >
