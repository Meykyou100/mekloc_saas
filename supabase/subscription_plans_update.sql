alter table public.agencies add column if not exists billing_type text not null default 'monthly' check (billing_type in ('monthly', 'annual'));
...
begin
  ...
  alter table public.agencies
    add constraint agencies_plan_check check (plan in ('starter', 'pro', 'business'));
end $$;
